		...
		for (int j = 0; j < fieldCnt; j++)
		{
			...
			if (j != index)
			{
				...
				else
				{
					...
					sprintf(buffer, "%d", condi.value_int);
					string temp(buffer);
					if (!(MakeData(fieldInfoList[j], (condi.type == 0) ? temp : condi.value_str, "UPDATE", data)))
						return false;
				}			
			}
			else
			{
				if (isNull)
					data = Data(NULL, 0);
				else if (!MakeData(fieldInfoList[j], target, "UPDATE", data))
					return false;
			}
...
bool QueryProcessor::InsertRecord(const string& tableName, const vector<string>& fieldNames, const vector<string>& values, const vector<bool>& isNulls)
{
	...
	for (int i = 0; i < fieldCnt; i++)
	{
		...
		{
			if (!(MakeData(fieldInfoList[i], values[index], "INSERT", data)))
				return false;
		}
...
bool QueryProcessor::MakeData(const FieldInfo& fieldInfo, const string& str, const string& opType, Data& data)
{
...
bool QueryProcessor::ConditionFilter(const string& tableName, const vector<FieldInfo>& fieldInfoList, const string& fieldName, const string& target, const string& op, Relation& relation)
{
	...
	{
		if (!MakeData(fieldInfoList[index], target, "WHERE", data))
			return false;
	}
	...
	if (!(op == "Null" || op == "not Null"))
	{
		if (fieldInfoList[index].fieldType == 0)
			...
			condi.value_str = string(RecordTool::data2Str(data));	
	}
